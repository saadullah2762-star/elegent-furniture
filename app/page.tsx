    ...
    <main ref={hero} className="bg-ink text-paper">
      ...
      <section className="hero-section relative flex min-h-screen flex-col overflow-hidden">
        {heroSlides.map((slide, index) => (
          <div
            key={slide.label}
            className="hero-image absolute inset-0 bg-cover bg-center transition-all duration-[1800ms] ease-out"
            style={{
              backgroundImage: `url('${slide.image}')`,
              opacity: index === heroIndex ? 1 : 0,
              transform: index === heroIndex ? 'scale(1.04)' : 'scale(1.10)',
            }}
          />
        ))}
        ...
        <div className="relative z-10 mt-auto px-6 pb-10 md:px-10 md:pb-14 lg:px-14">
          <div className="mb-5 flex items-center gap-3 text-[10px] tracking-[0.3em] uppercase text-white/75">
            <span>Karachi · Custom Furniture</span>
            <span className="h-px w-8 bg-white/40" />
            <span className="text-white/55">{heroSlides[heroIndex].label}</span>
          </div>
          ...
          <div className="mt-8 flex items-center gap-2">
            {heroSlides.map((slide, index) => <button key={slide.label} aria-label={`Show ${slide.label}`} onClick={() => setHeroIndex(index)} className={`h-px transition-all duration-500 ${index === heroIndex ? 'w-12 bg-white' : 'w-5 bg-white/35'}`} />)}
          </div>
        </div>
      </section>
