...
const heroSlides = [
  ...
  { image: 'https://images.unsplash.com/photo-1600607687920-4e2a09cf159d?auto=format&fit=crop&w=2400&q=92', label: 'Warm Interior' },
...
export default function Home() {
  ...
  useEffect(() => {
    const shuffled = [...heroSlides].sort(() => Math.random() - 0.5);
    let position = 0;
    const interval = window.setInterval(() => {
      position = (position + 1) % shuffled.length;
      const next = heroSlides.findIndex((slide) => slide.image === shuffled[position].image && slide.label === shuffled[position].label);
      if (next >= 0) setHeroIndex(next);
    }, 6500);
    ...
    <main ref={hero} className="bg-ink text-paper">
      <section className="hero-section relative flex min-h-screen flex-col overflow-hidden">
        {heroSlides.map((slide, index) => (
          <div key={`${slide.label}-${index}`} className="hero-image absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url('${slide.image}')`, opacity: index === heroIndex ? 1 : 0, transform: index === heroIndex ? 'scale(1)' : 'scale(1.055)', animation: index === heroIndex ? 'heroKenBurns 6.5s cubic-bezier(.16,1,.3,1) forwards' : 'none', transition: 'opacity 2400ms cubic-bezier(.22,1,.36,1), transform 2400ms cubic-bezier(.22,1,.36,1)', willChange: 'opacity, transform' }} />
        ))}
        ...
        <div className="relative z-10 mt-auto px-6 pb-10 md:px-10 md:pb-14 lg:px-14"><div className="mb-5 flex items-center gap-3 text-[10px] tracking-[0.3em] uppercase text-white/75"><span>Karachi · Custom Furniture</span><span className="h-px w-8 bg-white/40" /><span className="hero-label text-white/55">{heroSlides[heroIndex].label}</span></div><h1 className="max-w-5xl text-[10vw] font-light leading-[.9] tracking-[-.055em] md:text-[8vw]">Furniture,<br /><span className="italic">crafted for you.</span></h1><div className="mt-7 flex flex-col justify-between gap-7 md:flex-row md:items-end"><p className="max-w-sm text-sm leading-6 text-white/80">Custom furniture made for your home, your style, and your space.</p><a href="#collections" className="w-fit rounded-full bg-paper px-6 py-4 text-xs font-medium tracking-[.12em] text-ink uppercase">Explore collections <ArrowDown size={15} className="ml-3 inline" /></a></div><div className="mt-8 flex items-center gap-2">{heroSlides.map((slide, index) => <button key={`${slide.label}-${index}`} aria-label={`Show ${slide.label}`} onClick={() => setHeroIndex(index)} className={`h-px transition-all duration-700 ${index === heroIndex ? 'w-12 bg-white' : 'w-5 bg-white/35'}`} />)}</div></div>
        <style jsx>{`@keyframes heroKenBurns { 0% { transform: scale(1.055) translate3d(0,0,0); } 100% { transform: scale(1) translate3d(0,-0.35%,0); } } .hero-label { animation: labelFade 900ms ease both; } @keyframes labelFade { from { opacity: 0; transform: translateY(5px); } to { opacity: .55; transform: translateY(0); } }`}</style>
      </section>
