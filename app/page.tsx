...
// Curated real interior photography. The images overlap in tone so the transition feels like one continuous space.
const heroSlides = [
  { image: 'https://images.unsplash.com/photo-1600210492486-724fe5c67fb0?auto=format&fit=crop&w=2400&q=92', label: 'Living Rooms' },
  { image: 'https://images.unsplash.com/photo-1616486338812-3dadae4b4ace?auto=format&fit=crop&w=2400&q=92', label: 'Bedrooms' },
  { image: 'https://images.unsplash.com/photo-1556912173-46c336c7fd55?auto=format&fit=crop&w=2400&q=92', label: 'Kitchens' },
  { image: 'https://images.unsplash.com/photo-1618220179428-22790b461013?auto=format&fit=crop&w=2400&q=92', label: 'Wardrobes' },
  { image: 'https://images.unsplash.com/photo-1600607687920-4e2a09cf159d?auto=format&fit=crop&w=2400&q=92', label: 'Interiors' },
  { image: 'https://images.unsplash.com/photo-1497366811353-6870744d04b2?auto=format&fit=crop&w=2400&q=92', label: 'Office' },
  { image: 'https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?auto=format&fit=crop&w=2400&q=92', label: 'Craftsmanship' },
];
...
export default function Home() {
  ...
  useEffect(() => {
    const interval = window.setInterval(() => setHeroIndex((current) => (current + 1) % heroSlides.length), 7200);
    ...
    <main ref={hero} className="bg-ink text-paper">
      <section className="hero-section relative flex min-h-screen flex-col overflow-hidden">
        {heroSlides.map((slide, index) => (
          <div
            ...
            style={{
              ...
              transform: index === heroIndex ? 'scale(1)' : 'scale(1.045)',
              animation: index === heroIndex ? 'heroKenBurns 7.2s cubic-bezier(.16,1,.3,1) forwards' : 'none',
              transition: 'opacity 2200ms cubic-bezier(.22,1,.36,1), transform 2200ms cubic-bezier(.22,1,.36,1)',
              willChange: 'opacity, transform',
            }}
          />
        ))}
        <div className="absolute inset-0 bg-gradient-to-b from-black/48 via-black/10 to-black/72" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_55%_45%,transparent_0%,rgba(0,0,0,.12)_55%,rgba(0,0,0,.35)_100%)]" />
        ...
        <div className="relative z-10 mt-auto px-6 pb-10 md:px-10 md:pb-14 lg:px-14">
          <div className="mb-5 flex items-center gap-3 text-[10px] tracking-[0.3em] uppercase text-white/75">
            <span>Karachi · Custom Furniture</span><span className="h-px w-8 bg-white/40" />
            <span className="hero-label text-white/55">{heroSlides[heroIndex].label}</span>
          </div>
          ...
          <div className="mt-8 flex items-center gap-2">{heroSlides.map((slide, index) => <button key={slide.label} aria-label={`Show ${slide.label}`} onClick={() => setHeroIndex(index)} className={`h-px transition-all duration-700 ${index === heroIndex ? 'w-12 bg-white' : 'w-5 bg-white/35'}`} />)}</div>
        </div>

        <style jsx>{`
          @keyframes heroKenBurns {
            0% { transform: scale(1.055) translate3d(0, 0, 0); }
            100% { transform: scale(1) translate3d(0, -0.35%, 0); }
          }
          .hero-label { animation: labelFade 900ms ease both; }
          @keyframes labelFade {
